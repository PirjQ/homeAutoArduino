...
const char * c = password.cstr();
intx = 1; 
int calibrationTime = 10;
//the time when the sensor outputs a low impulse 
long unsigned int lowIn;
//theamountofmillisecondsthesensorhastobelow//beforeweassumeallmotionhasstopped 
...
void setup()
{
 ...
 writeReg(0x11, 10);
  ...
  BT.begin(9600); Serial.begin(9600); pinMode(pirPin, INPUT);
...
voidloop()
{
  while(BT.available())
  {
   ...
   delay(1000); 
   const char * d = readData.cstr();
   result = strcmp(c,d); 
   if(!result==1){ ﬂag=1; BT.write(”OK”); }
   if(ﬂag==1)
   {
    delay(500);
    if(digitalRead(pirPin) == HIGH)
    { 
      Serial.println(”Detected ”); //the led visualizes the sensors output pin state 
      BT.write(”detected”); 
      if(lockLow){ //makes sure we wait for a transition to LOWbeforeanyfurtheroutputismade: 
        lockLow=false;
        Serial.println(”—”);
        ...
        {
          Serial.println(”Safe ”); //the led visualizes the sensors output pin state 
          BT.write(”safe”); 
          if(takeLowTime)
          {
            lowIn=millis();//savethetimeofthetransitionfromhightoLOW
            takeLowTime = false; //make sure this is only done at the start of a LOW phase}//if the sensor is low formorethanthegivenpause,//weassumethatnomoremotionisgoingtohappen
            ...
            Serial.print((millis() - pause)/1000); Serial.println(” sec”); delay(500);}} delay(200); readData=””;} }
            }
